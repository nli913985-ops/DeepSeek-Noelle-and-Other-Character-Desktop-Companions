...
v1.1.0：更新检查访问本仓库 GitHub Releases API，下载仅由用户点击立即更新后发起。可配置 HTTPS 国内对象存储备用源，声明采用 Ed25519 签名，下载采用 SHA-256 校验。签名验证使用 cryptography（Apache-2.0 / BSD-3-Clause），版本比较使用 packaging（Apache-2.0 / BSD-2-Clause）。


v1.1.1 精简包：所有运行时动作帧继续保留在 assets/skins，各皮肤原图仅保留于制作工程；安装包不再重复携带生成原图与旧绿幕素材。上文素材路径中的 full-*、cycle-* 为历史制作来源。诺艾尔提示词与定位记录仍随包保留。硬件监测继续使用 SensorHost 与 LibreHardwareMonitorLib，省略独立 LibreHardwareMonitor 图形界面及调试文件；保留 PawnIO 驱动安装器。桌宠使用 QWidget/QPainter 平面绘制，省略未使用的 OpenGL 软件渲染器。

## v1.4.4 新约能天使

角色来源：《明日方舟》，角色权利归原权利人。本皮肤以用户提供的 assets 文件夹中的同人重绘图为基础，使用内置 image_gen 工具补画和修订完整动作帧，非官方游戏原始贴图。沿用提供素材的红发、光环、白色翼片和黑白红服装设定；运行时整帧切片、统一缩放及对齐，保留独立光环与翼片，不拼接程序绘制的肢体。原有空白横幅、重复动作和异常初稿不作为最终动作使用。使用内置图像生成路径，未使用 CLI/API 后备生成。

新增或修订行走、侧面攀爬、顶部悬挂、擦屏幕、原神横幅、WKCO 举旗、影分身结印。用户提供的待机、进食、睡眠、醒来等素材继续保留。完整生成提示词与定位参数随皮肤保留在 prompts.json 和 registration.json；原始制作图片存于制作工程，不重复放入安装包。
